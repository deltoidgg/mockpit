...
__export(index_exports, {
  MockPitDevtoolsElement: () => MockPitDevtoolsElement,
  MockPitMarkElement: () => MockPitMarkElement,
  MockPitSectionElement: () => MockPitSectionElement,
  defineAllMockPitElements: () => defineAllMockPitElements,
  defineMockPitElements: () => defineMockPitElements,
  defineMockPitMarkElement: () => defineMockPitMarkElement,
  defineMockPitSectionElement: () => defineMockPitSectionElement,
  mountMockPitDevtools: () => mountMockPitDevtools,
  registerAttributeMarks: () => registerAttributeMarks
});
...
var MockPitDevtoolsElement = class extends HTMLElement {
  ...
  renderTrigger(snapshot) {
    ...
    const label = snapshot ? `MockPit \xB7 ${capitalise(snapshot.mode)} \xB7 ${snapshot.summary.capture?.status === "blocked" ? "Blocked" : issueCount > 0 ? `${issueCount} issues` : (0, import_core.formatSourceMix)(snapshot.summary.sourceCounts)}` : "MockPit \xB7 No client";
...
var renderFilters = (filters, includeHighlight = false) => `
  <div class="filters" aria-label="Resource filters">
    <label>Source ${select("source", ["all", ...import_core.sourceKinds], filters.source)}</label>
    <label>Status ${select("status", ["all", ...import_core.auditStatuses], filters.status)}</label>
    <label>Scope ${select("scope", ["current", "all"], filters.scope)}</label>
    ${includeHighlight ? `<label>Highlight ${select("highlight", ["all", ...import_core.sourceKinds], filters.highlight)}</label>` : ""}
...
var renderRecordRow = (record) => `
  <article class="row">
    ...
    <p class="muted">${escapeHtml(record.label)} \xB7 ${import_core.statusLabels[record.status]} \xB7 ${record.visibility ?? "mounted"}</p>
...
// src/section.ts
var import_core2 = require("@mockpit/core");
var MockPitSectionElement = class extends HTMLElement {
  ...
  applyHighlight() {
    ...
    this.title = `${section.label}: ${(0, import_core2.formatSourceMix)(section.sourceCounts)}`;
  }
};
